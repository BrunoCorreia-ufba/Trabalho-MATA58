...
class Main_Window(QMainWindow):
    def __init__(self):
        ...
        openFile = QAction(QIcon('openThing.png'), '&Abrir', self)
        openFile.setShortcut('Ctrl+O')
        openFile.setStatusTip('Abrir arquivo')
        openFile.triggered.connect(self.file_open)

        saveFile = QAction(QIcon('saveThing.png'), '&Salvar', self)
        saveFile.setShortcut("Ctrl+S")
        saveFile.setStatusTip('Salvar arquivo')
        saveFile.triggered.connect(self.file_save)

        addProcess = QAction(QIcon('newThing.png'), '&NewProcess', self)
        addProcess.setShortcut('Ctrl+N')
        addProcess.setStatusTip('Novo processo')
        ...
        linhaMem = QLabel()
        linhaMem.setText("Algoritmo de paginação")
        ...
        override_label.setText("Sobrecarga")
        ...
        self.toolbar = self.addToolBar('Processos')
    ...
    def run(self):
        if self.listProcess == []:
            reply = QMessageBox.information(self, 'SEM PROCESSOS', "Insira processos para rodar", QMessageBox.Ok)
            return
        self.file_open(True)
    ...
    def file_save(self, auto=False):
        if not auto:
            ...
            name = "('" + self.path[:-12] + 'autosave' + "', " + "'Todos os arquivos (*)')"
    ...
    def removeProcess(self):
        ...
        reply = QMessageBox.question(self, 'Remover',
                                     "Quer mesmo remover? ", QMessageBox.Yes |
    ...
    def janelaUi(self, processo):
        ...
        layout.addRow("Tempo chegada", linhaStart)

        linhaExec = QLabel()
        linhaExec.setText(str(processo.execution_time))
        layout.addRow("Tempo execução", linhaExec)

        linhaDeadLine = QLabel()
        linhaDeadLine.setText(str(processo.deadline))
        layout.addRow("Deadline", linhaDeadLine)

        linhaPriority = QLabel()
        linhaPriority.setText(str(processo.priority))
        layout.addRow("Prioridade", linhaPriority)

        linhaPages = QLabel()
        linhaPages.setText(str(processo.numpages))
        layout.addRow("Páginas", linhaPages)

        btnEdit = QPushButton('Editar ' + str(self.numberTab) + ' ' + '° Processo', self.janela)
        btnRemove = QPushButton('Remover ' + str(self.numberTab) + ' ' + '° Processo', self.janela)
        ...
        self.lista.setTabText(self.numberTab, "Processo " + str(processo.id))
